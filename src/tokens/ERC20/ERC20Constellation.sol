...
/**
 *  @dev Thirdweb inspired ERC20Constellation contract using solidstates ERC20 contract and diamond storage throughout.
 *       has the edition of ERC165
 *
 *  The `ERC20Constellation` smart contract implements the ERC20 standard.
...
contract ERC20Constellation is
    SolidStateERC20,
    AccessControl,
    Multicall,
    ContractMetadata,
    ...
    ReentrancyGuard
{
    error ERC20Constellation_notAuthorized();
    error ERC20Constellation_zeroAmount();
    error ERC20Constellation_insufficientBalance();
    ...
    function validConstellation(bytes32 _implementationId) public view returns (bool) {
        return _getGlobals().getERC20Implementation(_implementationId) == address(this);
    }
    ...
    function mintTo(address _to, uint256 _amount) public payable virtual nonReentrant {
        if (!_canMint()) {
            revert ERC20Constellation_notAuthorized();
        }

        if (_amount < 1) {
            revert ERC20Constellation_zeroAmount();
        }
    ...
    function burn(uint256 _amount) external virtual {
        if (_balanceOf(msg.sender) < _amount) {
            revert ERC20Constellation_insufficientBalance();
        }
