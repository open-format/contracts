...
contract Governed {
    ...
    error Governed_notAuthorised();
    error Governed_invalidGovernorAddress();
    ...
    constructor(address _initGovernor) {
        if (_initGovernor == address(0)) {
            revert Governed_invalidGovernorAddress(); 
        }
    ...
    function acceptOwnership() external {
        if (pendingGovernor == address(0) || msg.sender != pendingGovernor) {
            revert Governed_notAuthorised();
        }
